-- Check and drop REG_CLUSTER_LOCK table if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reg_cluster_lock') THEN
...
-- Check and drop REG_LOG table if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'reg_log') THEN
        DROP TABLE REG_LOG;
    END IF;
END $$;

-- Check and drop REG_LOG_PK_SEQ sequence if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.sequences WHERE sequence_name = 'reg_log_pk_seq') THEN
        DROP SEQUENCE REG_LOG_PK_SEQ;
    END IF;
END $$;
